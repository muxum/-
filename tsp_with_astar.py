...
from 初始化地图 import CRgraph,CRedge
...
def tsp(graph,starter,edge,newnode):
    S = Solution(graph,starter)
    print("total:",S.tsp())
    # 开始回溯
    M = S.array
    #print(M)
    ...
    while len(lists) > 0:
        ...
        print(newnode[start],"--->" ,newnode[next_node],"edge:",edge[newnode[start]][newnode[next_node]],"cost:",graph[start,next_node])
        start = next_node
